...
export default {
  ...
  methods: {
    ...
    loadProduct() {
      useJwt.axiosIns.post(`http://127.0.0.1:8080/product/${this.id}`).then(res => {
        if (res.data.error) {
          this.showToast(res.data.error, 'danger')
          return
        }
        this.productData = res.data.product
        this.productDataOri = JSON.parse(JSON.stringify(res.data.product))
      })
    },
    submit() {
      ...
      useJwt.axiosIns.post(`http://127.0.0.1:8080/product/admin/${this.id}/update`, querystring.stringify(body)).then(res => {
        if (res.data.error) {
          this.showToast(res.data.error, 'danger')
          return
        }
        this.productData = res.data.product
        this.showToast('修改商品資料成功!', 'success')
      })
    },
